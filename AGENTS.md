...
`apps/gunmetal/` is the native Gunmetal CLI entrypoint; `apps/web/` holds the Next.js site and docs. The internal SDK lives in `packages/sdk/`, shared SDK-facing types live in `packages/sdk-core/`, and first-party provider extensions live in `packages/extensions/`. App-only Rust packages live under `packages/app-*`: `packages/app-cli` handles commands, `packages/app-daemon` serves HTTP and the local Web UI, and `packages/app-storage` owns SQLite/local state. `packages/npm/` is the npm distribution wrapper; executable code is in `packages/npm/bin`, tests in `packages/npm/test`.
