...
`apps/web/` holds the Next.js site and docs; app routes live in `apps/web/src/app`, shared UI in `apps/web/src/components`, and copy/content in `apps/web/src/lib`. `packages/cli/` is the npm wrapper; executable code is in `packages/cli/bin`, tests in `packages/cli/test`. Core product code is the Rust workspace under `crates/`: `gunmetal-app` boots the binary, `gunmetal-cli` handles commands, `gunmetal-daemon` serves HTTP, `gunmetal-providers` integrates upstreams, `gunmetal-storage` owns SQLite, `gunmetal-core` shares types, and `gunmetal-tui` renders the terminal UI.
...
- `bun run test` runs web type checks, CLI tests, and `cargo test --workspace`.
- `bun run check` runs web lint, CLI checks, `cargo fmt --check`, and strict Clippy.
- `cargo run -p gunmetal-app -- --help` smoke-tests the Rust entrypoint.
...
## Testing Guidelines
Follow test-first work. Start with a failing test, then implement. CLI changes should begin with `packages/cli/test/*.test.js`; Rust changes should add or extend nearby `#[cfg(test)]` modules. Run `bun run test` before opening a PR. Target full coverage for touched code; do not merge untested paths.
